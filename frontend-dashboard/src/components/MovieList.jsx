...
export function MovieList() {
  ...
  const handleColumnToggle = (column) => {
    setSelectedColumns((prevSelected) =>
      prevSelected.includes(column)
        ? prevSelected.filter((c) => c !== column) // Remove if already selected
        : [...prevSelected, column] // Add if not selected
    );
  };
  ...
  const downloadCSV = async (downloadType) => {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute("download", `movies_${downloadType}.csv`)
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mt-4 flex justify-between items-center">
        ...
        <div className="flex items-center">
          ...
          />
          <Button
            onClick={() => paginate(Number.parseInt(currentPage))}
            variant="outline"
          >
            Go
          </Button>
