...
const {DB_template_post, DB_template_mentions} = require("../instagram/templates") 
const {upload_to_bunny_cdn, saveCarouselMedia, DB_store_hashtags, DB_store_mentions} = require("../instagram/processTask")
  ...
  while (postList.length < post_limit) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        logger.info(`${postList.length}/${post_limit} ${type} fetched for ${username}`);
...
async function DB_store_mapping(post_id, user_id, movie_id, platform, trx){
    try {
        await trx('post_influencer_movie_map').insert({post_id, user_id, movie_id, platform})
    } catch (error) {
...
async function DB_store_post(trx, postData, task_id, user_id, followers_count) {
    ...
    **/
    try {
        ...
        const hashtags = postData['caption']['hashtags'].map((hashtag) => (hashtag.split('#')[1]));
        const mentions = postData['caption']['mentions'];
...
async function savePosts(postsData, user_id, task_id, followers_count){
    ...
    try {
        ...
        for (const post of postsData) {
            const DB_response = await DB_store_post(trx, post, task_id, user_id, followers_count);
            if (!DB_response.success) {
                await trx.rollback();
                logger.error(`🔴 Error storing Post at savePosts(task_id = ${task_id}, postid=${post.id}) | ${DB_response.message}`);
                return {status: 500, success: false, message: DB_response.message};
            }
            if(i%20 == 0){
              logger.info(`⚪ Successfully Stored ${i}/${postsData.length} posts(task_id = ${task_id})`);
            }
            i++;
        }
        await trx.commit();
        logger.info(`⚪ Success at savePosts(task_id = ${task_id}) | Data Stored Successfully`);
...
async function processTask(task_id, user_id, username, followers_count, post_limit) {
  try {
    // Fetch the user's posts
    const postFetchResponse = await fetchPosts(username, post_limit, "posts");
    if(postFetchResponse.status != 200){
        logger.info(`🔴 Error in fetching posts for ${username}, only ${postFetchResponse.data.length} posts fetched`);
