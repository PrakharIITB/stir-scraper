...
async function populateGenres(movieId, imdbData) {
    try {
        ...
        for (const genre of genres) {
            ...
            if(genreId)
                await db('movie_genres')
                    .insert({ movie_id: movieId, genre_id: genreId.id });
            else{
                ...
                if(existingGenre){
                    await db('movie_genres')
                        .insert({ movie_id: movieId, genre_id: existingGenre.id });
                }
            }
        }
...
async function populateSpokenLanguages(movieId, tmdbData) {
    try {
        ...
        for (const language of languages) {
            ...
            await db('movie_spoken_languages')
                .insert({ movie_id: movieId, language_code: iso_639_1 });
        }
...
async function populateTmdbProductionCompanies(movieId, tmdbData) {
    try {
        ...
        for (const company of companies) {
            ...
            if(companyId){
                await db('movie_tmdb_production_companies')
                    .insert({ movie_id: movieId, company_id: companyId.id });
            }
            else{
                ...
                if(existingCompany){
                    await db('movie_tmdb_production_companies')
                        .insert({ movie_id: movieId, company_id: existingCompany.id })
                }
...
async function populateImdbProductionCompanies(movieId, imdbData) {
    try {
        ...
        for (const company of companies) {
            ...
            // Insert the movie-company relationship
            if(companyId)
                await db('movie_imdb_production_companies')
                    .insert({ movie_id: movieId, company_id: companyId.id });
            else{
...
async function populateCrew(movieId, imdbData) {
    try {
        ...
        for (const member of crewMembers) {
            ...
            if(crewId)
                await db('movie_crew')
                    .insert({ movie_id: movieId, crew_id: crewId.id, character_name: characters });
            else{
                ...
                if (existingCrew) {
                    await db('movie_crew')
                        .insert({ movie_id: movieId, crew_id: existingCrew.id, character_name: characters });
                }
            }
        }
        //directors
        ...
        for (const director of directors) {
            ...
            if(directorId)
                await db('movie_crew')
                    .insert({ movie_id: movieId, crew_id: directorId.id });
            else{
                ...
                if (existingDirector) {
                    await db('movie_crew')
                        .insert({ movie_id: movieId, crew_id: existingDirector.id });
                }
            }
        }
        ...
        for (const writer of writers) {
            ...
            if(writerId)
                await db('movie_crew')
                    .insert({ movie_id: movieId, crew_id: writerId.id });
            else{
                ...
                if (existingWriter) {
                    await db('movie_crew')
                        .insert({ movie_id: movieId, crew_id: existingWriter.id });
                }
            }
        }
...
async function addMovie(movieTmdbId, movieImdbId) {
    try {
        ...
        const movieData = {
            ...
            origin_country: tmdbData.origin_country.join(", "),
            interests: imdbData.interests.join(", "),
        ...
        const sanitizedMovieData = Object.fromEntries(
            Object.entries(movieData).filter(([_, value]) => value !== undefined && value !== null)
        );
